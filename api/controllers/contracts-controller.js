...
const getUserByUsernameAndEcosystem = (username, ecosystemAddress) => new Promise((resolve, reject) => {
  log.debug(`REQUEST: Get user by username: ${username} in ecosystem at ${ecosystemAddress}`);
  ...
  ecosystem
    ...
    .then((data) => {
      ...
      log.info(`SUCCESS: Retrieved user address ${data.raw[0]} by username ${username} and ecosystem ${ecosystemAddress}`);
      return resolve({
        address: data.raw[0],
      });
    })
