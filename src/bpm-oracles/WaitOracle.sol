...
contract WaitOracle is Application {

    event LogWaitStarted(
    ...
    bytes32 constant EVENT_ID_WAIT_ORACLE = "AN://oracles/monax/time/wait";
    bytes32 constant MAPPING_ID_FREQUENCY = "Frequency";
     ...
     * This WaitOracle expects to have read access to a data mapping with the ID "Duration" containing a string that specifies
     * the time to wait. All information is then emitted via the LogWaitStarted event.
     * REVERTS if:
    ...
    function complete(address _piAddress, bytes32 _activityInstanceId, bytes32, address _txPerformer) public {
        string memory frequency = ProcessInstance(_piAddress).getActivityInDataAsString(_activityInstanceId, MAPPING_ID_FREQUENCY);
        eventLog[_activityInstanceId] = _piAddress;
        emit LogWaitStarted(EVENT_ID_WAIT_ORACLE, _activityInstanceId, address(this), address(bpmService), _piAddress, _txPerformer, block.timestamp, frequency, false);
    }

    /**
     * @dev Allows to complete the WaitOracle for a given UUID via an external call.
    ...
    function completeOracle(bytes32 _uuid) external {
        ErrorsLib.revertIf(eventLog[_uuid] == address(0),
            ErrorsLib.INVALID_INPUT(), "WaitOracle.completeOracle", "The provided ActivityInstance ID (UUID) is not registered");
        ...
        if (error == BaseErrors.NO_ERROR()) {
            ...
            emit LogWaitCompleted(EVENT_ID_WAIT_ORACLE, _uuid, address(this), true);
        }
        else {
            emit LogWaitCompleted(EVENT_ID_WAIT_ORACLE, _uuid, address(this), false);
        }
    }
